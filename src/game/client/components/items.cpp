...
void CItems::RenderProjectile(const CNetObj_Projectile *pCurrent, int ItemID)
{
	...
	if (g_Config.m_ClAntiPing)
	{
		...
		bool LocalPlayerInGame = m_pClient->m_aClients[m_pClient->m_LocalClientID].m_Team != -1;
		static float Offset = 1;
		Offset = mix(Offset, (float)(Client()->PredGameTick() - Client()->GameTick()), 0.05);


	//	if(pCurrent->m_Type == WEAPON_GRENADE && LocalPlayerInGame && ! m_pClient->m_Snap.m_pGameData->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER)
		if(LocalPlayerInGame && ! m_pClient->m_Snap.m_pGameData->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER)
		{
