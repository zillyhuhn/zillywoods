...
void CMenus::RenderMenubar(CUIRect Rect)
{
	...
	if((Client()->State() == IClient::STATE_OFFLINE && m_MenuPage == PAGE_SETTINGS) || (Client()->State() == IClient::STATE_ONLINE && m_GamePage == PAGE_SETTINGS))
	{
		...
		if(DoButton_MenuTabTop(&s_ZillyButton, Localize("Z"), Client()->State() == IClient::STATE_OFFLINE && Config()->m_UiSettingsPage==SETTINGS_ZILLY, &Button,
			Config()->m_UiSettingsPage == SETTINGS_ZILLY ? 1.0f : NotActiveAlpha, 1.0f, CUI::CORNER_ALL))
		{
			m_pClient->m_pCamera->ChangePosition(CCamera::POS_SETTINGS_ZILLY);
			Config()->m_UiSettingsPage = SETTINGS_ZILLY;
		}
	}
...
void CMenus::RenderLoading(int WorkedAmount)
{
	...
	TextRender()->TextColor(CUI::ms_DefaultTextColor);
	TextRender()->TextOutlineColor(CUI::ms_DefaultTextOutlineColor);
	UI()->DoLabel(&Rect, "ZillyWoods", 48.0f, CUI::ALIGN_CENTER);
...
void CMenus::SetActive(bool Active)
{
	...
	if(!m_MenuActive)
	{
		if(Client()->State() == IClient::STATE_ONLINE)
		{
			...
			{
				m_SkinModified = false;
				m_pClient->SendSkinChange(Config()->m_ClDummy);
			}
		}
	}
	else
