		...
		else
		{
			...
			for(int t = 0; t < 2; ++t)
			{
				...
				if(aPlayerInfo[t].m_pPlayerInfo)
				{
					...
					{
						if(m_pClient->IsRaceGametype() && Config()->m_ClDDRaceScoreBoard)
						{
				...
				{
					aBuf[0] = 0;
					// TODO: do we need to also reset based on ClDDRaceScoreBoard config?
...
void CHud::RenderTextInfo()
{
	...
	if(Config()->m_ClShowpred)
	{
		static CTextCursor s_Cursor(12);
		s_Cursor.m_Align = TEXTALIGN_RIGHT;
		s_Cursor.MoveTo(m_Width-10, Config()->m_ClShowfps ? 20 : 5);
