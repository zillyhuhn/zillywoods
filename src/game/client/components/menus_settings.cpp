...
void CMenus::RenderHSLPicker(CUIRect MainView)
{
	...
	if(DoButton_CheckBox(&s_CustomColors, Localize("Custom colors"), *CSkins::ms_apUCCVariables[Config()->m_ClDummy][m_TeePartSelected], &Button))
	{
		*CSkins::ms_apUCCVariables[Config()->m_ClDummy][m_TeePartSelected] ^= 1;
		m_SkinModified = true;
	}

	if(!(*CSkins::ms_apUCCVariables[Config()->m_ClDummy][m_TeePartSelected]))
		return;
	...
	int Color = *CSkins::ms_apColorVariables[Config()->m_ClDummy][m_TeePartSelected];
	...
	if(Modified)
	{
		...
		for(int p = 0; p < NUM_SKINPARTS; p++)
		{
			if(m_TeePartSelected == p)
				*CSkins::ms_apColorVariables[Config()->m_ClDummy][p] = NewVal;
		}
		if(UseAlpha)
...
void CMenus::RenderSkinSelection(CUIRect MainView)
{
	...
	for(int i = 0; i < s_paSkinList.size(); ++i)
	{
		...
		if((Config()->m_ClDummy && !str_comp(s->m_aName, Config()->m_PlayerSkin)) ||
			(!Config()->m_ClDummy && !str_comp(s->m_aName, Config()->m_DummySkin)))
		{
			m_pSelectedSkin = s;
			OldSelected = i;
		}
...
void CMenus::RenderSkinPartSelection(CUIRect MainView)
{
	...
	for(int i = 0; i < s_paList[m_TeePartSelected].size(); ++i)
	{
		...
		if(s == 0)
			continue;
		if(!str_comp(s->m_aName, CSkins::ms_apSkinVariables[Config()->m_ClDummy][m_TeePartSelected]))
			OldSelected = i;
		...
		if(Item.m_Visible)
		{
			...
			{
				int SkinPart = m_pClient->m_pSkins->FindSkinPart(j, CSkins::ms_apSkinVariables[Config()->m_ClDummy][j], false);
				const CSkins::CSkinPart *pSkinPart = m_pClient->m_pSkins->GetSkinPart(j, SkinPart);
				if(*CSkins::ms_apUCCVariables[Config()->m_ClDummy][j])
				{
					if(m_TeePartSelected == j)
						Info.m_aTextures[j] = s->m_ColorTexture;
					else
						Info.m_aTextures[j] = pSkinPart->m_ColorTexture;
					Info.m_aColors[j] = m_pClient->m_pSkins->GetColorV4(*CSkins::ms_apColorVariables[Config()->m_ClDummy][j], j==SKINPART_MARKING);
				}
				else
				{
					if(m_TeePartSelected == j)
...
void CMenus::RenderSkinPartPalette(CUIRect MainView)
{
	...
	for(int p = 0; p < NUM_SKINPARTS; p++)
	{
		...
		if(*CSkins::ms_apUCCVariables[Config()->m_ClDummy][p])
		{
			...
			{
				int& TeePartSelectedColor = *CSkins::ms_apColorVariables[Config()->m_ClDummy][m_TeePartSelected];
				TeePartSelectedColor = *CSkins::ms_apColorVariables[Config()->m_ClDummy][p];
...
void CMenus::RenderSettingsPlayer(CUIRect MainView)
{
	...
	DoEditBoxOption(Config()->m_DummyName, Config()->m_DummyName, sizeof(Config()->m_DummyName), &Button, Localize("Name"),  100.0f, &s_OffsetDummyName);

	// right menu
	Right.HSplitTop(ButtonHeight, &Button, &Right);
	static float s_OffsetDummyClan = 0.0f;
	DoEditBoxOption(Config()->m_DummyClan, Config()->m_DummyClan, sizeof(Config()->m_DummyClan), &Button, Localize("Clan"),  100.0f, &s_OffsetDummyClan);
...
void CMenus::RenderSettingsTee(CUIRect MainView)
{
	...
	if(!s_aPlayerSkin[0])
		str_copy(s_aPlayerSkin, Config()->m_ClDummy ? Config()->m_DummySkin : Config()->m_PlayerSkin, sizeof(s_aPlayerSkin));
	...
	if(!s_CustomSkinMenu)
	{
		...
		Preview.y += 2.0f;
		UI()->DoLabel(&Preview, Config()->m_ClDummy ? Config()->m_DummySkin : Config()->m_PlayerSkin, ButtonHeight*ms_FontmodHeight*0.8f, CUI::ALIGN_CENTER);
	}
...
void CMenus::RenderSettingsControls(CUIRect MainView)
{
	...
	Split = DoIndependentDropdownMenu(&s_ZillyDropdown, &MainView, Localize("ZillyWoods"), HeaderHeight, &CMenus::RenderSettingsControlsZilly, &s_ZillyActive);
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	else if(Config()->m_UiSettingsPage == SETTINGS_ZILLY)
